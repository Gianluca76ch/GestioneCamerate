...
exports.getAllAssegnazioni = async (req, res) => {
  try {
    ...
    if (matricola_alloggiato)
      whereClause.matricola_alloggiato = matricola_alloggiato;
...
exports.createAssegnazione = async (req, res) => {
  try {
    const { matricola_alloggiato, id_camera, data_assegnazione, note } =
      req.body;
    ...
    if (camera.agibile === false) {
      return res.status(400).json({
        success: false,
        error:
          "Camera non agibile: non è possibile assegnare militari a questa camera",
        numero_camera: camera.numero_camera,
        motivo: camera.manutenzione
          ? "Camera in manutenzione"
          : "Camera non agibile",
      });
    }
    ...
    const assegnazioneCompleta = await Assegnazione.findByPk(
      nuovaAssegnazione.id,
      {
        include: [
          {
            model: Alloggiato,
...
exports.spostaAlloggiato = async (req, res) => {
  try {
    ...
    if (cameraDestinazione.agibile === false) {
      return res.status(400).json({
        success: false,
        error:
          "Camera destinazione non agibile: non è possibile assegnare militari",
        numero_camera: cameraDestinazione.numero_camera,
      });
    }
    ...
    const nuovaAssegnazione = await Assegnazione.create({
      ...
      note:
        note ||
        `Spostato da camera ${assegnazioneCorrente.camera.numero_camera}`,
    });
